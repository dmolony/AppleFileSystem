...
// -----------------------------------------------------------------------------------//
{
  ...
  // ---------------------------------------------------------------------------------//
  public static void writeIntBigEndian (byte[] buffer, int ptr, int value)
  // ---------------------------------------------------------------------------------//
  {
    buffer[ptr + 3] = (byte) (value & 0xFF);
    buffer[ptr + 2] = (byte) ((value & 0xFF00) >>> 8);
    buffer[ptr + 1] = (byte) ((value & 0xFF0000) >>> 16);
    buffer[ptr] = (byte) ((value & 0xFF000000) >>> 24);
  }
  ...
  // ---------------------------------------------------------------------------------//
  {
    ...
    catch (DateTimeException e)
    {
      ...
      System.out.printf ("Date: %02X%02X%n", buffer[offset] & 0xFF,
          buffer[offset + 1] & 0xFF);
      System.out.printf ("Time: %02X%02X%n", buffer[offset + 2] & 0xFF,
  ...
  // ---------------------------------------------------------------------------------//
  {
    text.append (String.format (FMT, makeLabel (label), value1, value2));
  }
