...
import static com.bytezone.filesystem.ProdosConstants.FILE_TYPE_DIRECTORY;
import static com.bytezone.filesystem.ProdosConstants.SUBDIRECTORY;
import static com.bytezone.filesystem.ProdosConstants.SUBDIRECTORY_HEADER;
...
// -----------------------------------------------------------------------------------//
{
  ...
  // ---------------------------------------------------------------------------------//
  {
    ...
    for (int i = 0, prevBlockNo = 0; i < CATALOG_SIZE; i++)
    {
      ...
      Utility.writeShort (buffer, ptr, prevBlockNo);
      Utility.writeShort (buffer, ptr + 2, 0);

      if (prevBlockNo > 0)
        Utility.writeShort (buffer, prevBlockNo * BLOCK_SIZE + 2, blockNo);
  ...
  // ---------------------------------------------------------------------------------//
  {
    ...
    {
      Optional<FileEntry> fileEntryOpt = searchDirectory (catalogBlockNo, subdirectories[i]);
  ...
  // ---------------------------------------------------------------------------------//
  {
    ...
    // update links
    int ptr = blockNo * BLOCK_SIZE;
    Utility.writeShort (buffer, lastBlockNo * BLOCK_SIZE + 2, blockNo);   // point to next block
    Utility.writeShort (buffer, ptr, lastBlockNo);                        // point to previous block
